# Directives & Contraintes d'Environnement — 123fenetre.fr

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **123fenetre.fr** (123fenetre) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `123fenetre.fr` |
| **URL Cloudflare Pages** | `https://123fenetre.pages.dev` |
| **Dépôt GitHub** | `jc842/123fenetre` (branche principale : `main`) |
...
| **Matomo Site ID** | `78` |
| **Matomo MTM Container** | `container_F8nhI3vW.js` (Container ID : `F8nhI3vW`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `98` |
| **Zones Publicitaires** | Header (`602`), In-Content (`603`), Mobile Sticky (`599`), Large Rectangle (`598`), Half Page (`601`), Skyscraper (`600`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_F8nhI3vW.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 603).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 598).
